...
export default function VinylBackground(props: {vinylAmount: number, height?: number, discSize: [min: number, max: number], style?: string, discsStyle?: string, children?: preact.JSX.Element | preact.JSX.Element[]})
{
  ...
  for (const index of Array(props.vinylAmount)) 
  {
    ...
    result.push(<Disc size={size} style={`absolute top-0 left-0 ml-[${x}%] mt-[${y}vh] scale-[${gradientedSize}] opacity-[${(1 - yRandom) * 0.8}] -translate-1/2 ${props.discsStyle ?? ""}`} color={randomColor()}/>)
  }
